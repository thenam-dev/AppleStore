...
/**
 * DAO chỉ xử lý SQL và mapping dữ liệu (rule 2). Mỗi hàm tự mở/đóng Connection
 * riêng qua DBConnection.getConnection() (theo đúng form CategoryDAO) - Service
 * không truyền Connection xuống DAO nữa.
 *
 * Lưu ý: vì không còn 1 Connection/transaction xuyên suốt, các thao tác nhiều
 * bước (vd. checkout) không còn atomic - đây là lựa chọn được team chốt, chấp
 * nhận đánh đổi để đồng nhất pattern DAO toàn hệ thống.
 */
public class CartDAO {
    ...
    private static final String CART_ITEM_COLUMNS = """
        ...
        p.product_id, p.category_id, p.name AS product_name, pv.variant_label, pv.stock_quantity, pv.is_active,
    ...
    /**
     * Lấy cart_id của khách; tự tạo mới nếu khách chưa có giỏ hàng (1 customer
     * - 1 cart).
     */
    ...
    /**
     * Xoá 1 dòng khỏi giỏ, kèm điều kiện cart_id để tránh khách A xoá được dòng
     * của khách B.
     */
    ...
    /**
     * Đếm tổng số lượng sản phẩm trong giỏ - dùng hiển thị badge số lượng trên
     * header.
     */
    ...
    /**
     * Lấy toàn bộ giỏ hàng kèm thông tin sản phẩm/giá hiện tại để hiển thị
     * cart.jsp hoặc để checkout.
     */
